"""DDL for the customer price list table."""


def customer_price_list_ddl(output_config):
    """
    summary: Builds the CREATE EXTERNAL TABLE DDL statement for
        customer_price_list, resolving the write path from
        output_config.
    ...
    if full_table_name == f"{catalog}.{schema}.{table}":
        ddl_statements = """
            CREATE EXTERNAL TABLE IF NOT EXISTS {table_name} (
                ...
                ZREP STRING,
                ZREP_Description STRING,
                Price DOUBLE,
                Currency STRING,
                UOM STRING,
                Demand_Group_ID STRING,
                Demand_Group_Desc STRING,
                Start_Date STRING,
                End_Date STRING,
                Sales_Org STRING,
